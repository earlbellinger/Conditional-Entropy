...
def hc(bins, row, col, size):
        ...
        hc1 = [(bins[i][j]/len(size))*numpy.log(sum(bins[m_i][j]/len(size) for m_i in row)/(bins[i][j]/len(size))) for i in row for j in col]
        s = numpy.nansum(hc1)
...
for p in periods:
	...
	entropy.append( hc(bins, row, col, normalized.T[1]))
